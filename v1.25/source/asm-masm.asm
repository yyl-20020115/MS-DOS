...
code_data segment
   assume cs:code_data,ds:code_data,es:code_data
...
OBJECT  EQU   0100H  ;DEFAULT "PUT" ADDRESS
...
CONST_    EQU   1  ;Constant (including $)
...
HEADER:  
   DB 13,10,'Seattle Computer Products 8086 Assembler Version 2.44'
...
BEGIN:
   ...
   INT   33
   MOV   AL,[FCB+17]
   MOV   BYTE PTR[SYMFLG],AL ;Save symbol table request flag
   MOV   SI,FCB+9 ;Point to file extension
   LODSB        ;Get source drive letter
   ...
   MOV   [BX],AL
DEFAULT:
   LODSB        ;Get HEX file drive letter
   CMP   AL,'Z'      ;Suppress HEX file?
   JZ L0000
   CALL  CHKDSK
L0000:   
   MOV   BYTE PTR[HEXFCB],AL
   LODSB        ;Get PRN file drive letter
   ...
   MOV   AH,80H
NOPRN:
   MOV   BYTE PTR[LSTFCB],AL
   MOV   BYTE PTR[LSTDEV],AH ;Flag device for list ouput
   MOV   SI,OFFSET EXTEND
   MOV   DI,FCB+9
   MOVSW
   MOVSB        ;Set extension to ASM
   MOVSW        ;Zero extent field
   MOV   DX,FCB
   MOV   AH,OPEN
   INT   33
   MOV   BX,OFFSET NOFILE
   OR AL,AL
   JZ $+5
   JMP   PRERR
   MOV   DX,OFFSET HEXFCB
   CALL  MAKFIL
   MOV   DX,OFFSET LSTFCB
   CALL  MAKFIL
   XOR   AX,AX
   MOV   BX,FCB
   MOV   [BX+12],AX ;Zero CURRENT BLOCK field
   MOV   [BX+32],AL ;Zero Next Record field
   MOV   [BX+14],BUFSIZ   ;Set record size
;inits   
   MOV   [BUFPT], OFFSET SRCBUF ;Initialize buffer pointer
   MOV   [CODE],OFFSET START+1 ;POINTER TO NEXT BYTE OF INTERMEDIATE CODE
   MOV   [IY],OFFSET START  ;POINTER TO CURRENT RELOCATION BYTE
   XOR   AX,AX
...
MAKFIL:
   ...
   MOV   DI,SI
   MOV   SI,FCB+1
   REP MOVSW        ;Copy source file name
   MOV   AH,MAKE
   INT   33
   MOV   BYTE PTR[DI-9+14],1 ;Set record length to 1 byte
   MOV   BX,OFFSET NOSPAC
   OR AL,AL    ;Success?
   JNZ   PRERR
RET_1:
RET_23:
...
; in CL. OP code in CH. Returns only if immediate operation.
   ...
   CMP   AL,DH
   JZ SEGCHK
   MOV   AL,CONST_ ;Check if the first operand is immediate
   MOV   CL,26
   CMP   AL,BH
...
; UNDEF.
   ...
   JZ NOIND_    ;Jump if not indexed, with BASE bit in carry
   CMC
...
MODE:
   ...
   JNZ   RET_5     ;If not, use 16-bit displacement
   AND   CH,07FH     ;Reset 16-bit displacement
   OR CH,040H     ;Set 8-bit displacement
   OR BX,BX
   JNZ   RET_5      ;Use it if not zero displacement
...
NOIND_:
   ...
   JNC   RET_5      ;If no base register, that's right
   RCL   AL,1    ;Check BP bit
   JC MODE
...
PAREN:
   CALL  GETSYM      ;Eat the "("
   CALL  EXPRESSION
   CMP   [SYM],")" ;Better have closing paren
   MOV   CL,20
   JNZ   ERR30
   RET
...
;Get register number
   ...
   CMP   AL,CONST_ ;Better have found a constant
...
ASMLIN:
   MOV   [MAXFLG],1   ;Allow only B and W flags normally
   MOV   BX,[PC]
   MOV   [OLDPC],BX
   CALL  GETLAB
   JNC   $+5
   ...
   JC ERR5
   INC   BX
   CMP   BYTE PTR[BX],"f"  ;See if an 8087 mnemonic
   JZ NDPOP
   CMP   AL,5
   JNC   ERR5
   MOV   AL,[BX]
...
NDPOP:   ;First letter is "F" so must be 8087 opcode ("Numeric Data Processor")
   MOV   [MAXFLG],4   ;Allow all type flags
   INC   BX
   CMP   BYTE PTR[BX],"n"  ;"No-wait" form?
   MOV   AH,0
   JNZ   SAVNFLG
   MOV   AH,1
   DEC   AL
...
SAVNFLG:
   ...
   MOV   SI,DX
   OR BYTE PTR[SI+BX],80H  ;Set high bit of last character
   ...
   JA OPERR
   CBW
   SHL   AX,1    ;Double to index into address table
   XCHG  AX,SI    ;Put in index register
   LEA   DI,[SI+NDPTAB] ;Get start of opcode table for this letter
LOOKNDP:
...
SPECIALOP:
   ...
   JZ FWAIT_    ;If zero, go handle FWAIT
;Handle FNOP
   CMP   [NOWAIT],0   ;Was "N" present (If not opcode was "FOP")
   JZ OPERR
...
GRP19:   ;ESC opcode
   ...
   JZ ESCMEM
   CMP   AL,CONST_ ;Check for another immediate
   JZ ESCIMM
   MOV   CL,20
ERRJ: JMP   ERROR
...
WREND:
   ...
   DEC   DL
   JNZ   WREND
   MOV   [BUFPT],OFFSET SRCBUF
   MOV   [HEXCNT],-5  ;FLAG HEX BUFFER AS EMPTY
   MOV   [LSTPNT],OFFSET LSTBUF
   MOV   [HEXPNT],OFFSET HEXBUF
   XOR   AX,AX
   MOV   [ERRCNT],AX
   MOV   [PC],AX
   ...
   XOR   AX,AX
   MOV   BX,FCB
   MOV   [BX+12],AX ;Set CURRENT BLOCK to zero
   MOV   [BX+20H],AL   ;Set NEXT RECORD field to zero
   MOV   [BX+14],BUFSIZ
   MOV   [COUNT],AL
   MOV   CH,1
   MOV   SI,OFFSET START
FIXLINE:
   MOV   DI,OFFSET START ;Store code over used up intermediate code
   XOR   AL,AL
...
;Either a word or byte fixup is needed from a forward reference
   ...
   CMP   WORD PTR[BX],0 ;See if symbol got defined
   JNZ   HAVDEF
   MOV   [ERR],100 ;Undefined - flag error
   XOR   AX,AX
HAVDEF:
...
;Check for short jump. If so, we're out of range; otherwise we're OK
   CMP   DI,OFFSET START+1  ;Only one other byte on line?
   JNZ   OBJBT    ;Can't be short jump if not
   MOV   AL,BYTE PTR[START]  ;Get the first byte of this line
...
RNGERR:
   MOV   [ERR],101 ;Value out of range
   JP OBJBT
...
LIST:
   ...
   AND   AL,7FH
   MOV   DL,AL
   TEST  [LSTDEV],3   ;See if output goes to console
   JZ PRNCHK
   MOV   AH,2
   INT   33    ;Output to console
PRNCHK:
   TEST  [LSTDEV],4   ;See if output goes to printer
   JZ FILCHK
   MOV   AH,5
...
FILCHK:
   MOV   AL,DL
   POP   DX
   TEST  [LSTDEV],80H ;See if output goes to a file
   JZ LISTRET
   CALL  WRTBUF
LISTRET:
   POP   AX
...
SYMDMP:
   ...
   CMP   AL,'S'
   JNZ   ENDSYM
   MOV   AL,BYTE PTR[LSTDEV]
   OR AL,AL    ;Any output device for symbol table dump?
   JNZ   DOSYMTAB
   OR AL,1     ;If not, send it to console
   MOV   BYTE PTR[LSTDEV],AL
DOSYMTAB:
   MOV   BX,OFFSET SYMMES
   CALL  PRINT
   MOV   DX,[BASE]
   MOV   AL,DH
   OR AL,DL
   JZ ENDSYM
   MOV   [SYMLIN],SYMWID  ;No symbols on this line yet
   MOV   BX,[HEAP]
   MOV   SP,BX    ;Need maximum stack for recursive tree walk
   CALL  NODE
ENDSYM:
   TEST  [LSTDEV],80H ;Print listing to file?
   JZ EXIT
   MOV   AL,1AH
...
TABVAL:
   ...
   MOV   AL,[BX+3]
   CALL  PHEX
   CMP   [SYMLIN],0   ;Will any more fit on line?
   JZ NEXSYMLIN
   MOV   AL,9
   CALL  LIST
   JP RIGHTSON
...
REPERR:
   ...
   INC   [ERRCNT]
   PUSH  AX
   MOV   BX,OFFSET ERRMES   ;Print "ERROR"
   CALL  PRINT
   POP   AX
...
FLUSHBUF:
   MOV   CX,DI
   MOV   DX,OFFSET LSTBUF
   MOV   DI,DX
   SUB   CX,DX
   JZ RET_21      ;Buffer empty?
   MOV   AH,SETDMA
   INT   33
   MOV   DX,OFFSET LSTFCB
   MOV   AH,BLKWRT
...
ERRTAB:
   ...
   DB 2,"Index or base register must be BP, BX, SI, or DI"
   DB 3,"Only one base register (BX, BP) allowed"
   DB 4,"Only one index register (SI or DI) allowed"
